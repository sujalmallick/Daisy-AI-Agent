...
export const CompactPlayer: React.FC<CompactPlayerProps> = ({
  ...
  return (
    <div
      ...
      className="group relative flex flex-col justify-between p-2.5 rounded-2xl cursor-pointer select-none shadow-2xl backdrop-blur-2xl bg-[#0c0f14]/90 border border-white/10 hover:border-white/20 w-[340px] h-[76px] overflow-hidden touch-none"
    ...
    >
      ...
      {artworkUrl && (
        <div
          className="absolute -left-4 -top-4 w-24 h-24 rounded-full blur-xl opacity-40 pointer-events-none transition-opacity group-hover:opacity-60"
          style={{
      ...
      <div className="flex items-center gap-3 w-full">
        ...
        <div className="relative w-10 h-10 rounded-xl overflow-hidden shrink-0 border border-white/10 bg-zinc-900 shadow-md flex items-center justify-center">
          {artworkUrl ? (
            <img
              src={artworkUrl}
              alt={trackTitle}
          ...
          {isPlaying && (
            <div className="absolute inset-0 bg-black/40 flex items-center justify-center gap-0.5 pointer-events-none">
              ...
              <span className="w-0.5 bg-emerald-400 rounded-full animate-pulse h-3.5 delay-75" />
        ...
        <div className="flex-1 min-w-0 pr-1">
          <span className="block text-xs font-semibold text-white truncate tracking-tight" title={trackTitle}>
            {trackTitle}
          </span>
          <p className="text-[10.5px] text-zinc-400 truncate mt-0.5" title={trackArtist}>
            {trackArtist}
          </p>
        </div>
        ...
        <div className="flex items-center gap-1 shrink-0">
          <button
            ...
            className="w-7 h-7 rounded-full text-zinc-400 hover:text-white hover:bg-white/10 flex items-center justify-center transition cursor-pointer active:scale-90"
          ...
          <button
            ...
            className="w-8 h-8 rounded-full bg-white text-black hover:bg-emerald-400 hover:text-black flex items-center justify-center transition-all cursor-pointer shadow-md active:scale-90"
      ...
      {/* Bottom Row: Progress Bar + Time Stamp (matches media_1788986590473.png) */}
      <div className="flex items-center gap-2.5 w-full pt-1">
        <div className="flex-1 h-[3px] bg-white/[0.1] rounded-full overflow-hidden">
          <div
            className="h-full bg-emerald-400 transition-all duration-300 shadow-[0_0_6px_#34d399] rounded-full"
            style={{ width: `${progressPercent}%` }}
          />
        </div>
        <span className="text-[10px] text-zinc-400 font-mono tracking-tight shrink-0 select-none">
